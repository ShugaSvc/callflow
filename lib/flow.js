...
function flow(flowName,states,beginArgs){
        ...
        function joinStates(){
            for(var join in joinsRegister){
                for(var inputState in joinsRegister[join].inputStates){
                    if(!statesRegister[inputState]){
                        expectedPromises[inputState] = false;
                        var f = function(inputState){
                            return function(result){
                                self[inputState] = result;
                            }
                        }(inputState);

                        self["function_"+inputState] = f;
                        statesRegister[inputState] = {
                            code: f,
                            joins: []
                        }
                    }
    ...
    function makePhaseUpdatesAfterCall(stateName){
        ...
        function updateJoinsAfterCall(stateName){
            ...
            for(var p in expectedPromises){
                var vr = expectedPromises[p];
                var v = self[p];
                if(Q.isPromise(v) && !Q.isPromise(vr)){
                    expectedPromises[p] = v;
                    v.then(self.continue(p,"Fulfilling promise "+p)).catch(self.error);
                }
            }
            ...
            statesRegister[stateName].joins.forEach(function (joinName) {
                ...
                if(joinsRegister[joinName]['tryOnNextTick'] === false) {
                    ...
                    } catch(err){}; //TODO: strange, refactoring
    ...
    this.continue = function(){
        ...
        return addErrorTreatment(async function(){
            currentPhase = stateName;
